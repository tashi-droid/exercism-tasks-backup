...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    efficiencypercentage = (generatedpower/ theoretical_max_power) * 100

    if efficiencypercentage >= 80:
        return 'green'
    elif efficiencypercentage >= 60:
        return 'orange'
    elif efficiencypercentage >= 30:
        return 'red'
    else:
        return 'black'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    actual = temperature * neutrons_produced_per_second
    lowerlimit = threshold * 0.9
    higherlimit = threshold * 1.1

    if actual < threshold * 0.9:
        return 'LOW'
    elif actual >= lowerlimit and actual <= higherlimit:
        return 'NORMAL'
    else:
        return 'DANGER'